...
public class MaxNoOfKSumPairs {
    ...
    /**
     * Time Complexity = Beats 94.83%
     * Space Complexity = Beats 47.66%
     */
    public int maxOperations(int[] nums, int k) {
        int len = nums.length, i = 0, j = len - 1, val, count = 0;
        Arrays.sort(nums);
        while (i < j) {
            val = nums[i] + nums[j];
            if (val == k) {
...
//        int[] nums = new int[]{10, 90, 75, 77, 72, 77, 61, 68, 62, 79, 89, 68, 42, 67, 75, 71, 61, 3, 70, 82, 70, 67, 66, 79, 78, 84, 61, 63, 73, 80, 61, 15, 12, 3, 66, 42, 55, 85, 60, 23, 69, 61, 64, 58, 84, 77};
        System.out.println(new MaxNoOfKSumPairs().maxOperations(nums, 4));
    }
}
